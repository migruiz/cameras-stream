  ...
  function getSubject(eventInfo){
    ...
    switch(eventInfo.type) {
        ...
        case 'MOVEMENT_BEFORE_AND_AFTER':
            return `=?utf-8?Q?DOOR_OPEN_=E2=AD=95_MOVEMENT_BEFORE_=26_AFTER_at_${dateFormat(date, "h:MM TT")}?=`;         
        case 'EXITING':        
            return `=?utf-8?Q?=3C--=F0=9F=9A=AA_EXITING_at_${dateFormat(date, "h:MM TT")}?=`;         
        case 'ENTERING':
            return  `=?utf-8?Q?--=3E=F0=9F=8F=A1_ENTERING_at_${dateFormat(date, "h:MM TT")}?=`;         
        default:
      }
  }
